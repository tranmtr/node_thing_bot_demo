module.exports = function(RED) {
    function GyroscopeDataNode(config) {
        ...
        node.on('input', function(msg) {
            ...
            if (!payload || !Array.isArray(payload.gyros) || payload.gyros.length !== 3) {
                node.warn("Dữ liệu gyroscope không hợp lệ hoặc bị thiếu trong msg.payload. Payload hiện tại: " + JSON.stringify(payload));
                return;
            }
            ...
            dataEntry.dataType = "gyros";
            dataEntry.x = payload.gyros[0] ? payload.gyros[0].toFixed(2) : "N/A";
            dataEntry.y = payload.gyros[1] ? payload.gyros[1].toFixed(2) : "N/A";
            dataEntry.z = payload.gyros[2] ? payload.gyros[2].toFixed(2) : "N/A";
    ...
    RED.nodes.registerType("gyroscope-filter-ui-table", GyroscopeDataNode);
};
